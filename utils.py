...
def test_single_volume(image, label, net, classes, dataset_id, predict_head, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            net.eval()
            with torch.no_grad():
                outputs = net(input, dataset_id, predict_head)
        ...
        net.eval()
        with torch.no_grad():
            out = torch.argmax(torch.softmax(net(input, dataset_id, predict_head), dim=1), dim=1).squeeze(0)
            prediction = out.cpu().detach().numpy()
    metric_list = []
    for i in range(1, classes):
        metric_i = calculate_metric_percase(prediction == i, label == i)
        # print('utils',type(metric_i))
